...
LEFT= [0, 0]
RESOLUTION = 0.5
COLOR = ['Red','Orange','Green','Cyan','Blue','Purple','Magenta','Yellow','White','Black','Gray','Brown','Pink','Turquoise','Gold','Silver']
...
def main():
    ...
    for i in range (ROBOT_COUNT): # For each robot of that type (given by the count in SIMULATION.yaml)
        ...
        cli_args2 = [PATH+'/launch/oneSet.launch', 'color:='+COLOR[i%len(COLOR)],'robot_name:='+str(i), 'pose_x:='+str(start[0]), 'pose_y:='+str(start[1]),'goal_x:='+str(end[0]),'goal_y:='+str(end[1])]
